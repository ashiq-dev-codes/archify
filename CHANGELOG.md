...
* Both architecture presets are now properly layered instead of a beginner-level shortcut:
  ...
  * Both presets share new dependency-free `core/error` (`Failure`/`Result<T>` sealed classes — no `dartz`/`fpdart` needed) and `core/network` (`NetworkInfo` via a plain `dart:io` DNS lookup — no `connectivity_plus` needed) infrastructure. DDD also gets `core/usecase`'s `UseCase<ReturnType, Params>` base contract.
  * The `feature_injection`/`injection_container` opt-in templates were updated to match: GetIt wires data source → repository → use case → Cubit, and `injection_container.dart` registers the shared `NetworkInfo` singleton.
