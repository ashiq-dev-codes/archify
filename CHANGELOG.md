...
* `init` now scaffolds `archify.yaml` from a chosen architecture preset instead of always writing the DDD default: `dart run archify init --arch mvvm`, or omit `--arch` to be prompted (Enter keeps DDD). Added a proper MVVM preset alongside the existing DDD one — `model`/`viewmodel`/`view` per feature, with a `ChangeNotifier` view model wired to its view via `ListenableBuilder`, no state-management package required. More architectures can be added to the `architecturePresets` registry later.
